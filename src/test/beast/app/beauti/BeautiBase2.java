 ...
 * 
 */
public class BeautiBase2 extends AssertJSwingJUnitTestCase {
	...
	void makeSureXMLParses() {
		...
		File XMLFile = new File(Files.temporaryFolder() + "/x.xml");
		if (XMLFile.exists()) {
			XMLFile.delete();
		}
...
//		fileChooser.setCurrentDirectory(Files.temporaryFolder());
//		fileChooser.selectFile(new File("x.xml")).approve();
		
		XMLParser parser = new XMLParser();
		XMLFile = new File(Files.temporaryFolder() + "/x.xml");
		try {
